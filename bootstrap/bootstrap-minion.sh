...
if [[ ${#pkg_new[@]} -gt 0 ]]; then
    ...
    apt-get install -y ${pkg_new[@]} >> "${apt_log}"
    ret_code=$?
    if [[ ! ${ret_code} ]]; then
       echo "'apt-get install -y ${pkg_new[@]}' exited with status code ${ret_code}" >> "${apt_log}"
       ...
       echo "skipped $((apt_log_lines_new-apt_log_lines_old)) lines"
       exit 4
    fi
    echo -e "\n"
fi
